...
var (
	//pod_inst      = "/opt/los/pods/a44624e689474ca9-0000/home/action/.los/pod_instance.json"
...
func do() {
	...
	var (
		proc_reload = false
		nss         = map[string]losapi.NsPodServiceMap{}
	)

	for _, app := range inst.Apps {

		if app.Spec.Meta.Name != "los-httplb" {
			continue
		}
		...
		for _, res := range app.Operate.Options {
			...
			for _, bound := range res.Items {
				...
				case "pod":
					...
					if !ok {
						...
						if err := json.DecodeFile("/dev/shm/los/nsz/"+vs[0], &nsz); err != nil {
							continue
						}
					...
					if len(bups) > 0 {
						upsname := fmt.Sprintf("losnsz_%s_%s_%d",
							domain,
							vs[0],
							port,
						)
				...
				case "upstream":
					...
					if len(bups) > 0 {
						upsname := fmt.Sprintf("losups_%s_%s",
							domain,
			...
			ngx_conf := "# generated by ngxkeeper\n"
